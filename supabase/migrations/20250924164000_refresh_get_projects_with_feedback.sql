DO $$
DECLARE
  has_projects boolean := to_regclass('public.projects') IS NOT NULL;
  has_orgs boolean := to_regclass('public.organizations') IS NOT NULL;
  has_ratings boolean := to_regclass('public.project_rating_summaries') IS NOT NULL;
  has_comments boolean := to_regclass('public.project_comment_summaries') IS NOT NULL;
BEGIN
  IF NOT (has_projects AND has_orgs AND has_ratings AND has_comments) THEN
    RAISE NOTICE 'Skipping refresh_get_projects_with_feedback: prerequisite objects not available.';
    RETURN;
  END IF;
